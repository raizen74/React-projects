...
// you define default values for the context, better autocompletion
...
export function OpinionsContextProvider({ children }) {
  ...
  useEffect(() => {
    async function loadOpinions() {
      const response = await fetch("http://172.30.210.50:3000/opinions");
  ...
  async function upvoteOpinion(id) {
    const response = await fetch(
      "http://172.30.210.50:3000/opinions/" + id + "/upvote",
      {
        method: "POST",
      }
    );
  ...
  async function downvoteOpinion(id) {
    const response = await fetch(
      "http://172.30.210.50:3000/opinions/" + id + "/downvote",
      {
        method: "POST",
      }
    );
